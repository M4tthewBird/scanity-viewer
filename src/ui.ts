...
const initUI = (global: Global) => {
    ...
    const dom = [
        ...
        'infoOrbitDesktop',
        'infoFlyDesktopClick',
        'infoFlyDesktopGaming',
        'infoWalkDesktopClick',
        'infoWalkDesktopGaming',
        'infoOrbitTouch',
        'infoFlyTouchClick',
        'infoFlyTouchGaming',
        'infoWalkTouchClick',
        'infoWalkTouchGaming',
        'infoPanelRememberInput',
        'infoPanelCta',
        'walkHint',
        'reset',
        'frame',
    ...
    // NOTE: keep these in sync with index.scss — $clr-accent (scan-blue, the
    // fill) and the #loadingBar track color (ink at low opacity)
    const loadingBarColor = '#246BFD';
    const loadingBarTrack = 'rgba(16,17,18,0.08)';
    events.on('progress:changed', (progress) => {
        dom.loadingText.textContent = `${progress}%`;
        if (progress < 100) {
            dom.loadingBar.style.backgroundImage = `linear-gradient(90deg, ${loadingBarColor} 0%, ${loadingBarColor} ${progress}%, ${loadingBarTrack} ${progress}%, ${loadingBarTrack} 100%)`;
        } else {
            dom.loadingBar.style.backgroundImage = `linear-gradient(90deg, ${loadingBarColor} 0%, ${loadingBarColor} 100%)`;
        }
    });
    ...
    // Curated quick-start content: only the controls for whichever camera
    // mode + variant (gaming/click-to-move) is actually active are shown,
    // not an exhaustive reference of every mode. Kept live via
    // cameraMode:changed / gamingControls:changed so if the panel is opened
    // before the scene's real starting mode is known (e.g. the automatic
    // first-load popup, shown before collision data resolves), it corrects
    // itself once CameraManager sets the real mode rather than staying wrong.
    const infoModeBlocks = [
        dom.infoOrbitDesktop,
        dom.infoFlyDesktopClick,
        dom.infoFlyDesktopGaming,
        dom.infoWalkDesktopClick,
        dom.infoWalkDesktopGaming,
        dom.infoOrbitTouch,
        dom.infoFlyTouchClick,
        dom.infoFlyTouchGaming,
        dom.infoWalkTouchClick,
        dom.infoWalkTouchGaming
    ];

    const updateInfoContent = () => {
        infoModeBlocks.forEach((el) => el.classList.add('hidden'));

        const gaming = state.gamingControls;
        let desktopBlock = dom.infoOrbitDesktop;
        let touchBlock = dom.infoOrbitTouch;

        if (state.cameraMode === 'fly') {
            desktopBlock = gaming ? dom.infoFlyDesktopGaming : dom.infoFlyDesktopClick;
            touchBlock = gaming ? dom.infoFlyTouchGaming : dom.infoFlyTouchClick;
        } else if (state.cameraMode === 'walk') {
            desktopBlock = gaming ? dom.infoWalkDesktopGaming : dom.infoWalkDesktopClick;
            touchBlock = gaming ? dom.infoWalkTouchGaming : dom.infoWalkTouchClick;
        }

        desktopBlock.classList.remove('hidden');
        touchBlock.classList.remove('hidden');
    };

    events.on('cameraMode:changed', updateInfoContent);
    events.on('gamingControls:changed', updateInfoContent);
    updateInfoContent();
    ...
    // Any dismissal path marks the guide as seen (skipping the automatic
    // popup on future visits) unless the visitor unchecks "don't show this
    // again" — which defaults checked, matching the guide's original
    // always-once behavior for anyone who doesn't touch it.
    const closeInfoPanel = () => {
        dom.infoPanel.classList.add('hidden');
        if ((dom.infoPanelRememberInput as HTMLInputElement).checked) {
    ...
    dom.infoPanel.addEventListener('pointerdown', closeInfoPanel);
    dom.infoPanelClose.addEventListener('click', closeInfoPanel);
    dom.infoPanelCta.addEventListener('click', closeInfoPanel);

    // Auto-open the help panel immediately, the first time this browser
    // visits, so new visitors have something useful to read (desktop
    // controls or touch gestures, whichever applies) during the scene's
    // download instead of just watching the loading bar. Dismissing it —
    // the X button, the CTA, click outside, or Esc — marks it seen per the
    // remember checkbox above, so it won't reappear on later visits/reloads.
    if (localStorage.getItem('hasSeenHelp') === null) {
        updateInfoTab(state.inputMode);
