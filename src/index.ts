...
const main = async (canvas: HTMLCanvasElement, settingsJson: unknown, config: Config) => {
    ...
    // Recover automatically from a stalled first load. Confirmed via a real
    // production DevTools capture: the content fetch (fired eagerly in
    // index.html, before the engine bundle even finishes loading) completes
    // fully at the network level — status 200, full byte count — but the
    // engine's asset pipeline never reads/processes it: no progress event
    // ever fires, no error, just silence. A manual page refresh has always
    // fixed it. Automate that same fix: if there's still been zero progress
    // after a wait well past what a working load ever takes (confirmed
    // network-complete around 6-7s in the failing capture), reload once.
    // Gated on progress === 0 (not "still loading") so a large scene that's
    // genuinely, slowly downloading is left alone; gated on the
    // sessionStorage flag so a persistently broken load doesn't reload forever.
    const STALL_RECOVERY_KEY = 'sse-stall-recovery';
    const stallTimeout = setTimeout(() => {
        if (state.progress === 0 && !sessionStorage.getItem(STALL_RECOVERY_KEY)) {
            console.warn('[sse] content load stalled with zero progress — auto-reloading once to recover');
            sessionStorage.setItem(STALL_RECOVERY_KEY, '1');
            location.reload();
        } else if (state.progress === 0) {
            console.warn('[sse] content load stalled again, but the one-shot auto-recovery was already used this session');
        }
    }, 9000);
    events.on('loaded:changed', () => {
        clearTimeout(stallTimeout);
        sessionStorage.removeItem(STALL_RECOVERY_KEY);
    });
