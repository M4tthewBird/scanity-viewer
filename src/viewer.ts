...
class Viewer {
    ...
    constructor(
        ...
        Promise.all([gsplatLoad, skyboxLoad, collisionLoad]).then((results) => {
            ...
            // A walkable collision mesh means the scene is meant to be
            // walked through — default straight into gaming controls
            // (WASD/mouse-look on desktop, joystick on touch) instead of
            // making the visitor discover and enable it manually. Walk mode
            // itself already defaults on in this case (see CameraManager).
            // On desktop this also tries to engage pointer lock immediately;
            // browsers require an actual user gesture for that, so this first
            // attempt is silently rejected and PointerLockManager reverts the
            // flag back off (existing behavior — see pointer-lock.ts). That's
            // fine: without gaming controls, desktop walk mode still works
            // fully via click-to-walk-to-point navigation, so nobody is
            // blocked from walking around; a desktop visitor who specifically
            // wants WASD/mouse-look still has to turn it on themselves via
            // the G shortcut (the settings-panel toggle is touch-only).
